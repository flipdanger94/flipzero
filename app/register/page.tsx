...
function RegisterForm() {
  ...
  return <AuthShell title="Создайте аккаунт" description="Ваше новое пространство начинается здесь." alternateText="Уже есть аккаунт?" alternateHref={safeNext ? `/login?next=${encodeURIComponent(safeNext)}` : "/login"} alternateLabel="Войти">
    <form className="auth-form" onSubmit={submit}>
      ...
      <label><span>Пароль</span><div className="auth-input"><LockKeyhole size={17} /><input name="password" value={password} onChange={(event) => setPassword(event.target.value)} type={showPassword ? "text" : "password"} autoComplete="new-password" placeholder="Минимум 10 символов" minLength={10} required /><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Скрыть пароль" : "Показать пароль"}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></label>
      <small className="auth-password-hint" role="status">{password.length ? password.length < 10 ? `Ещё ${10-password.length} символов до минимальной длины` : "Длина пароля подходит. Не используйте один пароль на разных сайтах." : "Пароль должен содержать не менее 10 символов."}</small>
      <label><span>Повторите пароль</span><div className="auth-input"><LockKeyhole size={17} /><input name="confirmPassword" type={showConfirmation ? "text" : "password"} autoComplete="new-password" placeholder="Повторите пароль" minLength={10} required /><button type="button" onClick={() => setShowConfirmation((value) => !value)} aria-label={showConfirmation ? "Скрыть повтор пароля" : "Показать повтор пароля"}>{showConfirmation ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></label>
      <label className="auth-consent"><input name="acceptedTerms" type="checkbox" required /><span>Я принимаю <a href="/terms" target="_blank" rel="noopener noreferrer">правила сервиса</a> и <a href="/privacy" target="_blank" rel="noopener noreferrer">политику конфиденциальности</a>.</span></label>

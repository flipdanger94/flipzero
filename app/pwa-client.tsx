...
export function PwaClient() {
  ...
  useEffect(() => {
    ...
    if (!standalone && Date.now() - dismissedAt > WEEK && isIos) window.setTimeout(() => setShowInstall(true), 0);
  ...
  return <>{!online ? <div className="offline-toast" role="status"><WifiOff size={15} /> Нет соединения. Ждём восстановления сети…</div> : null}{showInstall ? <aside className="install-app-card" aria-label="Установить FlipZero"><button className="install-close" onClick={dismiss} aria-label="Закрыть"><X size={15} /></button><span className="install-mark">FZ</span><div><strong>Установить FlipZero</strong><small>{showIosHelp ? <>Нажмите <Share size={12} /> «Поделиться», затем «На экран Домой».</> : "Откройте чаты как отдельное приложение."}</small></div><button className="install-action" onClick={showIosHelp ? dismiss : install}><Download size={15} /> {showIosHelp ? "Понятно" : "Установить"}</button></aside> : null}</>;
}

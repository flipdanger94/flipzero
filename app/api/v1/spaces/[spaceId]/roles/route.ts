...
async function requireRoleManager(spaceId: string) {
  ...
  return { database, user, space, owner: true, permissions: 0, topPosition: Number.MAX_SAFE_INTEGER };
}
...
export async function POST(request: Request, { params }: { params: Promise<{ spaceId: string }> }) {
  ...
  if (!canGrantRolePermissions(access.owner, access.permissions, parsed.data.permissions) || (!access.owner && access.topPosition <= 0)) return NextResponse.json({ code: "ROLE_HIERARCHY", message: "Нельзя создать роль с такими правами или уровнем." }, { status: 403 });
  const [position] = await access.database.select({ value: max(roles.position) }).from(roles).where(eq(roles.spaceId, spaceId));
  const [created] = await access.database.insert(roles).values({ id: randomUUID(), spaceId, ...parsed.data, position: access.owner ? Math.max(1, (position.value ?? 0) + 1) : access.topPosition - 1, isManaged: false }).returning();
...
export async function PATCH(request: Request, { params }: { params: Promise<{ spaceId: string }> }) {
  ...
  if (typeof body?.id === "string" && typeof body?.showInMemberList === "boolean") {
    const [existing] = await access.database.select({ id: roles.id, position: roles.position }).from(roles).where(and(eq(roles.id, body.id), eq(roles.spaceId, spaceId))).limit(1);
    if (!existing) return NextResponse.json({ code: "NOT_FOUND", message: "Роль не найдена." }, { status: 404 });
    if (!canManageRole(access.owner, access.topPosition, existing.position)) return NextResponse.json({ code: "ROLE_HIERARCHY", message: "Нельзя изменять роль своего уровня или выше." }, { status: 403 });
  ...
  if (!canGrantRolePermissions(access.owner, access.permissions, parsed.data.permissions)) return NextResponse.json({ code: "ROLE_HIERARCHY", message: "Нельзя выдавать права выше своих." }, { status: 403 });
...
export async function DELETE(request: Request, { params }: { params: Promise<{ spaceId: string }> }) {
  ...
  if (!canManageRole(access.owner, access.topPosition, existing.position)) return NextResponse.json({ code: "ROLE_HIERARCHY", message: "Нельзя удалить роль своего уровня или выше." }, { status: 403 });

...
export async function GET(){
 ...
 const stories=await db.select({story:userStories,name:users.displayName,avatarUrl:users.avatarUrl}).from(userStories).innerJoin(users,eq(users.id,userStories.userId)).where(and(inArray(userStories.userId,ids),gt(userStories.expiresAt,new Date()),gt(userStories.createdAt,new Date(Date.now()-86400_000)))).orderBy(desc(userStories.createdAt)).limit(100);
 ...
 const storyIds=visible.map(row=>row.story.id);const [views,reactions]=await Promise.all([storyIds.length?db.select({storyId:storyViews.storyId,userId:storyViews.userId,name:users.displayName}).from(storyViews).innerJoin(users,eq(users.id,storyViews.userId)).where(inArray(storyViews.storyId,storyIds)):[],storyIds.length?db.select({storyId:storyReactions.storyId,emoji:storyReactions.emoji,userId:storyReactions.userId}).from(storyReactions).where(inArray(storyReactions.storyId,storyIds)):[]]);
 return NextResponse.json({stories:visible.map(({story,name,avatarUrl})=>({...story,hiddenUserIds:undefined,name,avatarUrl,viewCount:story.userId===user.id?views.filter(view=>view.storyId===story.id).length:undefined,viewers:story.userId===user.id?views.filter(view=>view.storyId===story.id).map(({userId,name})=>({userId,name})):undefined,viewed:views.some(view=>view.storyId===story.id&&view.userId===user.id),reactions:reactions.filter(reaction=>reaction.storyId===story.id).map(({emoji,userId})=>({emoji,userId}))}))});
}

...
export async function POST(request:Request,{params}:{params:Promise<{clanId:string}>}){
 ...
 const token=new AccessToken(key,secret,{identity:user.id,name:user.displayName,ttl:"2h",metadata:JSON.stringify({clanId,username:user.username,avatarUrl:user.avatarUrl,accentColor:user.accentColor})});
 token.addGrant({roomJoin:true,room:`clan:${clanId}`,canPublish:true,canPublishSources:[TrackSource.MICROPHONE,TrackSource.CAMERA,TrackSource.SCREEN_SHARE,TrackSource.SCREEN_SHARE_AUDIO],canSubscribe:true,canUpdateOwnMetadata:true});

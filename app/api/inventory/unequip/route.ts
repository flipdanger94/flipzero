...
import { StoreActionError, unequipInventorySlot } from "@/lib/store";
...
export async function POST(request: Request) {
  ...
  if (!slot) return NextResponse.json({ code: "INVALID_SLOT", message: "Слот не указан." }, { status: 400 });
  try {
    return NextResponse.json({ ok: true, inventory: await unequipInventorySlot(user.id, slot) });
  } catch (error) {

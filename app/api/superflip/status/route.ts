...
export async function GET() {
  ...
  return NextResponse.json({ status, active, capabilities: active ? SUPERFLIP_CAPABILITIES : STANDARD_CAPABILITIES, source: grant?.source ?? null, expiresAt: grant?.expiresAt ?? null, waitlisted: Boolean(waitlist), price: { amount: 4.99, currency: "USD", interval: "month" }, availability: "coming_soon" });
}

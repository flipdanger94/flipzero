...
import { quietHours } from "@/lib/dnd";
...
export async function GET(){
 ...
 const quiet=await quietHours(user.id);
 const unread=visibleRows.reduce((total,row)=>total+(row.readAt||quiet.active&&!quiet.favorites.includes(row.actorId??"")&&!(quiet.clanIds.includes(row.entityId??"")&&row.entityType==="clan")?0:1),0);
 return NextResponse.json({notifications:items,unread,quiet:quiet.active});
}

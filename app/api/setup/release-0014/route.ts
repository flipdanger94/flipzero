...
export async function POST() {
  ...
  const migration = await readFile(fileURLToPath(new URL("../../../../drizzle/0014_space_join_requests.sql", import.meta.url)), "utf8");
  const { default: postgres } = await import("postgres");
  const client = postgres(process.env.DATABASE_URL, { max: 1, prepare: false });
  try { await client.unsafe(migration); } finally { await client.end(); }

  return NextResponse.json({ ok: true, message: "Заявки на вступление установлены." });
}

...
export async function POST() {
  ...
  const migration = await readFile(fileURLToPath(new URL("../../../../drizzle/0015_space_audit_logs.sql", import.meta.url)), "utf8");
  const { default: postgres } = await import("postgres");
  const client = postgres(process.env.DATABASE_URL, { max: 1, prepare: false });
  try { await client.unsafe(migration); } finally { await client.end(); }

  return NextResponse.json({ ok: true, message: "Журнал действий сервера установлен." });
}

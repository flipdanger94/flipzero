...
export async function GET(request: Request) {
  ...
  const rows=await db.select({id:users.id,username:users.username,displayName:users.displayName,avatarUrl:users.avatarUrl,presence:users.presence,profileStatus:users.profileStatus})

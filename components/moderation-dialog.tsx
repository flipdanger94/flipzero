...
type Capabilities = { reviewFlags: boolean; warn: boolean; timeout: boolean; kick: boolean; ban: boolean; unban: boolean };
const emptyCapabilities: Capabilities = { reviewFlags: false, warn: false, timeout: false, kick: false, ban: false, unban: false };
...
export function ModerationDialog({ spaceId, onClose }: { spaceId: string; onClose: () => void }) {
  ...
  const allowedActions = useMemo(() => (Object.entries(labels) as [Case["action"], string][]).filter(([value]) => capabilities[value]), [capabilities]);
  const hasMemberActions = allowedActions.length > 0;
  const targets = action === "unban" ? bannedPeople : members.filter((member) => member.userId !== ownerId);

  useEffect(() => {
    const controller = new AbortController();
    fetch(`/api/v1/spaces/${spaceId}/moderation`, { signal: controller.signal }).then(async (response) => ({ response, data: await response.json() })).then(({ response, data }) => { if (!response.ok) throw new Error(data.message); setMembers(data.members); setCases(data.cases); setFlags(data.flags ?? []); setOwnerId(data.ownerId); setCapabilities(data.capabilities ?? emptyCapabilities); const firstAllowed = (Object.keys(labels) as Case["action"][]).find((value) => data.capabilities?.[value]); if (firstAllowed && !data.capabilities?.[action]) setAction(firstAllowed); if (!data.capabilities?.reviewFlags && firstAllowed) setSection("actions"); }).catch((reason) => { if (reason.name !== "AbortError") setError(reason.message ?? "Не удалось загрузить журнал."); }).finally(() => setLoading(false));

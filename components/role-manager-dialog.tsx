...
type Role = { id: string; name: string; color: string; position: number; permissions: number; isManaged: boolean; showInMemberList: boolean };
...
export function RoleManagerDialog({spaceId, onClose }: { spaceId: string; onClose: () => void }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    setSaving(true); setError("");
    const payload = selected.isManaged
      ? { id: selected.id, name: form.get("name"), color: form.get("color") }
      : { id: selected.id, name: form.get("name"), color: form.get("color"), permissions };
    const response = await fetch(`/api/v1/spaces/${spaceId}/roles`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
    const data = await response.json(); setSaving(false);
  ...
  async function setRoleVisibility(role: Role, showInMemberList: boolean) {
    setSaving(true); setError("");
    try {
      const response = await fetch(`/api/v1/spaces/${spaceId}/roles`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: role.id, showInMemberList }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.message ?? "Не удалось обновить роль.");
      setItems((current) => current.map((item) => item.id === role.id ? data.role : item));
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Не удалось обновить роль."); }

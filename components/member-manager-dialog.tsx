...
type Member = { userId: string; username: string; displayName: string; nickname: string | null; level: number; joinedAt: string; roleIds: string[] };
...
export function MemberManagerDialog({ spaceId, onClose }: { spaceId: string; onClose: () => void }) {
  ...
  useEffect(() => { fetch(`/api/v1/spaces/${spaceId}/members`).then(async (response) => ({ response, data: await response.json() })).then(({ response, data }) => { if (!response.ok) throw new Error(data.message); setItems(data.members); setRoles(data.roles); setOwnerId(data.ownerId); setSelectedId(data.members[0]?.userId ?? null); setDraftRoles(data.members[0]?.roleIds.filter((id: string) => data.roles.some((role: Role) => role.id === id && !role.isManaged)) ?? []); }).catch((reason) => setError(reason.message ?? "Не удалось загрузить участников.")).finally(() => setLoading(false)); }, [spaceId]);
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="space-dialog member-dialog" role="dialog" aria-modal="true" aria-labelledby="members-title"><button className="dialog-close" onClick={onClose} aria-label="Закрыть"><X size={19} /></button><div className="dialog-symbol"><Users size={22} /></div><h2 id="members-title">Участники</h2><p>Назначайте роли и управляйте доступом к пространству.</p>{error ? <div className="auth-error" role="alert">{error}</div> : null}{loading ? <div className="role-loading"><LoaderCircle className="spin" size={22} /> Загружаем участников...</div> : <div className="member-manager-layout"><aside className="managed-member-list">{items.map((member) => <button key={member.userId} className={member.userId === selectedId ? "active" : ""} onClick={() => selectMember(member)}><i>{member.displayName.split(/\s+/).map((part) => part[0]).join("").slice(0, 2).toUpperCase()}</i><span><strong>{member.displayName}</strong><small>@{member.username} · уровень {member.level}</small></span>{member.userId === ownerId ? <b>Владелец</b> : null}</button>)}</aside><div className="member-role-editor">{selected ? <><div className="member-editor-head"><div><strong>{selected.displayName}</strong><span>В сообществе с {new Date(selected.joinedAt).toLocaleDateString("ru-RU")}</span></div></div><h3>Пользовательские роли</h3>{selected.userId === ownerId ? <div className="role-protected"><Users size={30} /><strong>Владелец пространства</strong><span>Его роли и доступ защищены.</span></div> : <><div className="member-role-grid">{customRoles.length ? customRoles.map((role) => <label key={role.id}><input type="checkbox" checked={draftRoles.includes(role.id)} onChange={() => toggleRole(role.id)} /><i style={{ background: role.color }} /><span>{role.name}</span></label>) : <p>Сначала создайте пользовательскую роль.</p>}</div><div className="role-actions"><button className="danger-button" onClick={removeMember} disabled={working}><UserMinus size={15} /> Исключить</button><button className="auth-submit" onClick={saveRoles} disabled={working}><Save size={15} /> Сохранить роли</button></div></>}</> : <div className="role-protected"><Users size={30} /><strong>Участников пока нет</strong></div>}</div></div>}</section></div>;
}

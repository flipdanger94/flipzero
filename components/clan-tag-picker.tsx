...
const icons={shield:Shield,crown:Crown,swords:Swords,flame:Flame,zap:Zap,skull:Skull,star:Star,orbit:Orbit,moon:MoonStar,laurel:Trophy};
const palette=["#8B77FF","#57B6FF","#48CD9A","#FFC76E","#FF708B","#F294F0"];
export function ClanTagPicker({tag,initialColor="#8B77FF",initialIcon="shield",unlockedIcons=0,seasonWinner=false}:{tag:string;initialColor?:string;initialIcon?:string;unlockedIcons?:number;seasonWinner?:boolean}){
  ...
  return <fieldset ref={root} className="clan-tag-picker"><legend>Оформление тега</legend>
    ...
    <div className="clan-tag-icons">{Object.entries(icons).map(([value,Icon])=>{const locked=value==="orbit"&&unlockedIcons<1||value==="moon"&&unlockedIcons<2||value==="laurel"&&!seasonWinner;return <button type="button" key={value} className={icon===value?"selected":""} onClick={()=>setIcon(value)} disabled={locked} title={locked?value==="laurel"?"Награда топ-3 сезона":"Открывается улучшением казны":undefined} aria-label={`Значок ${value}`} aria-pressed={icon===value}><Icon size={19}/></button>})}</div>

...
export function UserDock({
  ...
  useEffect(()=>{
    ...
    const closeOutside=(event:PointerEvent)=>{const target=event.target;if(target instanceof Node&&!rootRef.current?.contains(target))setMenu(null)};
    ...
    return()=>{cancelled=true;document.removeEventListener("pointerdown",closeOutside);document.removeEventListener("keydown",closeEscape)};
  },[menu]);
  ...
  return <div ref={rootRef} className="user-dock">
    ...
    <div className="dock-audio-control">
      ...
      <button type="button" className="dock-chevron" onClick={()=>setMenu(menu==="input"?null:"input")} aria-label="Настройки микрофона" title="Настройки микрофона" aria-expanded={menu==="input"}><ChevronUp size={13}/></button>
    </div>
    ...
    <div className="dock-audio-control">
      ...
      <button type="button" className="dock-chevron" onClick={()=>setMenu(menu==="output"?null:"output")} aria-label="Настройки наушников" title="Настройки наушников" aria-expanded={menu==="output"}><ChevronUp size={13}/></button>
    </div>

    <button type="button" onClick={onOpenSettings} aria-label="Настройки аккаунта" title="Настройки аккаунта"><Settings size={17}/></button>

    {menu==="input"?<section className="dock-device-menu dock-device-menu-input" role="dialog" aria-label="Настройки микрофона">
    ...
    {menu==="output"?<section className="dock-device-menu dock-device-menu-output" role="dialog" aria-label="Настройки наушников">

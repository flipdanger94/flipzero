...
export function SpaceSettingsDialog({ space, onClose, onSaved, onRoles, onMembers, onInvites, onModeration, onProgress }: Props) {
  ...
  const [section, setSection] = useState<"profile" | "superup" | "people" | "moderation">("profile");
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section ref={dialogRef} tabIndex={-1} className="space-dialog space-settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title">
      ...
      <div className="space-settings-content">
        ...
        </> : section === "superup" ? <><div className="space-settings-heading"><span>SUPERUP · {space.name}</span><h2 id="settings-title">Бонусы SuperUp</h2><p>Планируемые уровни поддержки для участников этого пространства. Поддержка пока не запущена, поэтому счётчик и пожертвования недоступны.</p></div><div className="space-superup-levels"><article><b>Уровень 1</b><strong>Старт</strong><span>План: 2 поддержки</span><p>Дополнительные эмодзи и качество голоса.</p></article><article><b>Уровень 2</b><strong>Рост</strong><span>План: 7 поддержек</span><p>Оформление пространства и расширенные загрузки.</p></article><article><b>Уровень 3</b><strong>Максимум</strong><span>План: 14 поддержек</span><p>Дополнительные лимиты и персонализация.</p></article></div><p className="space-superup-note">Пороги и бонусы уточняются перед запуском. Уже загруженные баннеры и текущие возможности пространства сохраняются.</p></> : section === "people" ? <><div className="space-settings-heading"><span>ЛЮДИ</span><h2 id="settings-title">Участники и роли</h2><p>Управление составом и доступом к пространству.</p></div><div className="space-settings-actions">{onMembers ? <button onClick={onMembers}><Users size={20} /><span><strong>Участники</strong><small>Список, управление и исключение участников</small></span></button> : null}{onRoles ? <button onClick={onRoles}><UserRound size={20} /><span><strong>Роли и права</strong><small>Разрешения и бейджи рядом с именами</small></span></button> : null}{onInvites ? <button onClick={onInvites}><ClipboardList size={20} /><span><strong>Приглашения</strong><small>Ссылки и доступ по приглашению</small></span></button> : null}</div></> : <><div className="space-settings-heading"><span>БЕЗОПАСНОСТЬ</span><h2 id="settings-title">Модерация</h2><p>Инструменты для защиты сообщества.</p></div><div className="space-settings-actions">{onModeration ? <button onClick={onModeration}><ShieldCheck size={20} /><span><strong>Журнал и действия модерации</strong><small>Жалобы и принятые меры</small></span></button> : null}{onProgress ? <button onClick={onProgress}><Gem size={20} /><span><strong>Прогресс и достижения</strong><small>Настройка достижений и рейтинг</small></span></button> : null}</div></>}
      </div>
    </section>
  </div>;
}

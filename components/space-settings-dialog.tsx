...
export function SpaceSettingsDialog({ space, onClose, onSaved, onRoles, onMembers, onInvites, onModeration, onProgress }: Props) {
  ...
  const [section, setSection] = useState<"profile" | "superup" | "people" | "analytics" | "template" | "sounds" | "moderation">("profile");
  ...
  async function loadSounds() { const response = await fetch(`/api/v1/spaces/${space.id}/sounds`); const data = await response.json(); if (!response.ok) throw new Error(data.message ?? "Ошибка загрузки звуков."); setSounds(data.sounds); }
  useEffect(() => { if (section === "sounds") void fetch(`/api/v1/spaces/${space.id}/sounds`).then((response) => response.json()).then((data) => setSounds(data.sounds ?? [])).catch(() => setError("Не удалось загрузить звуки.")); }, [section, space.id]);
  async function uploadSound(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setLoading(true); setError(""); try { const form = event.currentTarget; const response = await fetch(`/api/v1/spaces/${space.id}/sounds`, { method: "POST", body: new FormData(form) }); const data = await response.json(); if (!response.ok) throw new Error(data.message ?? "Не удалось загрузить звук."); form.reset(); await loadSounds(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Ошибка загрузки."); } finally { setLoading(false); } }
  async function deleteSound(id: string) { setError(""); try { const response = await fetch(`/api/v1/spaces/${space.id}/sounds?id=${encodeURIComponent(id)}`, { method: "DELETE" }); if (!response.ok) throw new Error("Не удалось удалить звук."); await loadSounds(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Ошибка удаления."); } }
  ...
  async function exportTemplate() { setError(""); try { const response = await fetch(`/api/v1/spaces/${space.id}/template`); if (!response.ok) throw new Error("Не удалось получить шаблон."); const blob = await response.blob(); const link = document.createElement("a"); link.href = URL.createObjectURL(blob); link.download = `flipzero-template-${space.id}.json`; link.click(); URL.revokeObjectURL(link.href); } catch (reason) { setError(reason instanceof Error ? reason.message : "Ошибка выгрузки."); } }
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section ref={dialogRef} tabIndex={-1} className="space-dialog space-settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title">
      ...
      <aside className="space-settings-nav" aria-label="Разделы настроек">
        ...
        <small>РЕАКЦИИ</small>
        <button type="button" className={section === "sounds" ? "active" : ""} onClick={() => setSection("sounds")}><Settings2 size={17} /> Звуковая панель</button>
        <small>ЛЮДИ</small>
        ...
        <button type="button" className={section === "template" ? "active" : ""} onClick={() => setSection("template")}><ClipboardList size={17} /> Шаблон пространства</button>

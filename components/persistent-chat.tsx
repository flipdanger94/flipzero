...
type ChatMessage = { clan?:ClanTagData|null; id: string; authorId: string; displayName: string; username: string; avatarUrl?: string | null; content: string; attachments?: Attachment[]; replyToId: string | null; editedAt?: string | null; pinnedAt?: string | null; createdAt: string; reactions: Array<{ emoji: string; userId: string }> };
...
export function PersistentChat({ channelId, channelName, spaceId, currentUserId, ownerId, searchQuery, onOpenDirect }: { channelId: string; channelName: string; spaceId: string; currentUserId: string; ownerId?: string; searchQuery: string; onOpenDirect?: (userId:string)=>void }) {
  ...
  async function load(query = searchQuery) { const response = await fetch(`/api/v1/channels/${channelId}/messages${query.trim() ? `?q=${encodeURIComponent(query.trim())}` : ""}`); const data = await response.json(); if (response.ok) { setMessages(data.messages ?? []); setCanSend(data.permissions?.canSend !== false); setCanReact(data.permissions?.canReact !== false); setCanAttach(data.permissions?.canAttach !== false); } else setError(data.message ?? "Не удалось загрузить сообщения."); setLoading(false); }
  useEffect(() => {
    ...
    async function refresh() {
      ...
      try {
        ...
        if (response.ok) { setMessages(data.messages ?? []); setCanSend(data.permissions?.canSend !== false); setCanReact(data.permissions?.canReact !== false); setCanAttach(data.permissions?.canAttach !== false); setError(""); }

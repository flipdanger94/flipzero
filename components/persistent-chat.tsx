  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      try {
        ...
        if (response.ok) { const firstPage=historyRef.current.length===0;setMessages(current=>{const latest=(data.messages??[]) as ChatMessage[];const existing=historyRef.current.length?historyRef.current:current;const byId=new Map(existing.map(item=>[item.id,item]));for(const item of latest)byId.set(item.id,item);const combined=[...byId.values()].sort((a,b)=>new Date(a.createdAt).getTime()-new Date(b.createdAt).getTime());historyRef.current=combined;return combined});if(firstPage)setOlderCursor(data.nextCursor??null); setCanSend(data.permissions?.canSend !== false); setCanReact(data.permissions?.canReact !== false); setCanAttach(data.permissions?.canAttach !== false); setError(""); }

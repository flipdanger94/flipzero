...
export function ClanHub({currentUserId,onOpenDirect}:{currentUserId:string;onOpenDirect?:(userId:string)=>void}) {
  ...
  useEffect(()=>{
    ...
    return()=>window.removeEventListener("flipzero:clan-selected",select);
  },[]);
  useEffect(()=>{
    ...
    return()=>window.removeEventListener("flipzero:open-clan-voice",openVoice);
  },[detail?.clan.id]);
  ...
  return <section className="clan-hub">
    <header className="clan-hub-header"><div><small>FLIPZERO CLANS</small><h2>{leaderboard?"Рейтинг кланов":detail?detail.clan.name:"Поиск кланов"}</h2><p>{detail?`[${detail.clan.tag}] · ${detail.clan.name}`:"Создавайте команды, общайтесь и развивайте своё сообщество."}</p></div><div className="clan-header-actions">{detail?<span className="clan-role-pill"><Shield size={14}/>Ваша роль: {roleLabel[detail.role]}</span>:null}</div></header>
    ...
    <div className="clan-section-layout">
      <nav className="clan-section-nav" aria-label="Разделы кланов">
        <span>КЛАНЫ</span>
        <button type="button" className={leaderboard?"active":""} onClick={()=>setLeaderboard(true)}><Swords size={16}/>Рейтинг</button>
        {detail?<><button type="button" className={!leaderboard&&tab==="governance"?"active":""} onClick={()=>{setLeaderboard(false);setTab("governance")}}><Crown size={16}/>Мой клан</button><button type="button" className={!leaderboard&&tab==="chat"?"active":""} onClick={()=>{setLeaderboard(false);setTab("chat")}}><MessageCircle size={16}/>Клановый чат</button><button type="button" className={!leaderboard&&tab==="voice"?"active":""} onClick={()=>{setLeaderboard(false);setTab("voice")}}><Headphones size={16}/>Клановый войс</button><button type="button" className={!leaderboard&&tab==="members"?"active":""} onClick={()=>{setLeaderboard(false);setTab("members")}}><Users size={16}/>Участники</button>{canModerate?<button type="button" className={!leaderboard&&tab==="requests"?"active":""} onClick={()=>{setLeaderboard(false);setTab("requests")}}><BadgeCheck size={16}/>Заявки{detail.requests.length?<b>{detail.requests.length}</b>:null}</button>:null}{canManage?<button type="button" className={!leaderboard&&tab==="settings"?"active":""} onClick={()=>{setLeaderboard(false);setTab("settings")}}><Settings2 size={16}/>Настройки клана</button>:null}</>:<button type="button" className={!leaderboard?"active":""} onClick={()=>setLeaderboard(false)}><Search size={16}/>Поиск кланов</button>}
      </nav>
      <div className="clan-section-content">

    {leaderboard?<div className="clan-leaderboard">
      <div className="clan-rank-toolbar">
        <form className="clan-rank-search" onSubmit={event=>{event.preventDefault();setRankPage(1);setRankQuery((event.currentTarget.elements.namedItem("rankSearch") as HTMLInputElement).value.trim())}}>
          <Search size={17}/><input name="rankSearch" aria-label="Поиск кланов в рейтинге" placeholder="Название или тег" defaultValue={rankQuery}/><button className="clan-primary">Найти</button>
        </form>
        <span className="clan-rank-total">{rankTotal.toLocaleString("ru-RU")} кланов</span>
      </div>
      {publicClan?<section className="clan-public-card" style={publicClan.bannerUrl?{backgroundImage:`linear-gradient(90deg,#101827dc,#101827bb),url("${publicClan.bannerUrl}")`}:undefined}><button type="button" onClick={()=>{setFocusClanId(null);setPublicClan(null)}} aria-label="Закрыть карточку клана"><X size={17}/></button><ClanTag preview clan={publicClan}/><h3>{publicClan.name}</h3><p>{publicClan.description||"Описание клана пока не заполнено."}</p><span>Уровень {publicClan.level} · {publicClan.xp.toLocaleString("ru-RU")} XP · {publicClan.memberCount}/50 участников</span></section>:null}
      {myRank&&!rankRows.some(row=>row.id===myRank.id)?<div className="clan-own-rank"><span className="clan-own-badge">Ваш клан</span><b>Место #{myRank.rank}</b><ClanTag clan={myRank} variant="inline"/><span>{myRank.xp.toLocaleString("ru-RU")} XP</span></div>:null}
      <div className="clan-leaderboard-list">
        {rankRows.map(row=><button type="button" key={row.id} className={`clan-rank-card ${row.rank<=3?"top-three":""} ${row.id===myRank?.id?"is-own":""} ${row.id===focusClanId?"is-focused":""}`} onClick={()=>setFocusClanId(row.id)}>
          <b className="clan-rank">{row.rank===1?"🥇":row.rank===2?"🥈":row.rank===3?"🥉":`#${row.rank}`}</b>
          <span className="clan-rank-avatar">{row.avatarUrl?<MediaImage src={row.avatarUrl}/>:row.tag.slice(0,2).toLocaleUpperCase("ru")}</span>
          <span className="clan-rank-identity"><span><ClanTag clan={row}/>{row.id===myRank?.id?<em className="clan-own-badge">Ваш клан</em>:null}</span><strong>{row.name}</strong></span>
          <span className="clan-rank-stat"><small>Уровень</small><b>{row.level}</b></span>
          <span className="clan-rank-stat"><small>XP</small><b>{row.xp.toLocaleString("ru-RU")}</b></span>
          <span className="clan-rank-stat clan-rank-members"><small>Участники</small><b><Users size={14}/>{row.memberCount}/50</b></span>
        </button>)}
        {!rankRows.length?<div className="clan-empty clan-rank-empty"><Swords size={30}/><strong>Кланы не найдены</strong><span>{rankQuery?"Попробуйте другой запрос или очистите поиск.":"Рейтинг появится здесь, когда кланы начнут набирать XP."}</span></div>:null}
      </div>
      {rankTotal>20?<nav className="clan-rank-pages"><button disabled={rankPage===1} onClick={()=>setRankPage(rankPage-1)}>Назад</button><span>Страница {rankPage} из {Math.max(1,Math.ceil(rankTotal/20))}</span><button disabled={rankPage*20>=rankTotal} onClick={()=>setRankPage(rankPage+1)}>Далее</button></nav>:null}
    </div>:!detail?<div className="clan-discovery">

...
export function ClanHub({currentUserId,onOpenDirect}:{currentUserId:string;onOpenDirect?:(userId:string)=>void}) {
  ...
  return <section className="clan-hub">
    ...
    {leaderboard?<div className="clan-leaderboard">
      ...
      {myRank?<div className="clan-own-rank"><span className="clan-own-badge">Ваш клан</span><b>#{myRank.rank}</b><ClanTag clan={myRank} variant="full"/><span>{myRank.xp.toLocaleString("ru-RU")} XP</span></div>:null}
      <div className="clan-leaderboard-list">
        ...
        {!rankRows.length?<div className="clan-empty clan-rank-empty"><Swords size={30}/><strong>Кланы не найдены</strong><span>{rankQuery?"Попробуйте другой запрос или очистите поиск.":"Рейтинг появится здесь, когда кланы начнут набирать XP."}</span></div>:rankRows.length<5?<div className="clan-rank-short-hint">Пока в рейтинге немного кланов. Новые команды появятся здесь по мере набора XP.</div>:null}
      </div>
      <nav className="clan-rank-pages"><button disabled={rankPage===1} onClick={()=>setRankPage(rankPage-1)}>Назад</button><span>Страница {rankPage} из {Math.max(1,Math.ceil(rankTotal/20))}</span><button disabled={rankPage*20>=rankTotal} onClick={()=>setRankPage(rankPage+1)}>Далее</button></nav>

...
export function EventsDialog({ spaceId, onClose }: { spaceId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    fetch(`/api/v1/spaces/${spaceId}/events`).then(async (response) => ({ response, data: await response.json() })).then(({ response, data }) => { if (!response.ok) throw new Error(data.message); setItems(data.events ?? []); setCanManage(Boolean(data.canManage)); }).catch((reason) => setError(reason.message ?? "Не удалось загрузить события.")).finally(() => setLoading(false));
  }, [spaceId]);

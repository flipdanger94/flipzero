...
export function ChannelPermissionsDialog({ spaceId, channel, categories, onClose, onUpdated }: { spaceId: string; channel: { id: string; name: string; parentId?: string | null }; categories: Array<{ id: string; name: string }>; onClose: () => void; onUpdated?: (channel: { id: string; name: string; parentId: string | null; topic: string | null; kind: string; slowmodeSeconds: number; isNsfw: boolean }) => void }) {
  ...
  const [channelSettings, setChannelSettings] = useState({ name: channel.name, parentId: channel.parentId ?? null, topic: "", slowmodeSeconds: 0, isNsfw: false });
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/spaces/${spaceId}/channel-overrides?channelId=${encodeURIComponent(channel.id)}`, { signal: controller.signal }).then(async (response) => ({ response, data: await response.json() })).then(({ response, data }) => {
      ...
      setRoles(data.roles); setItems(data.overrides); setSelectedId(data.roles[0]?.id ?? null); setChannelSettings({ name: data.channel?.name ?? channel.name, parentId: data.channel?.parentId ?? null, topic: data.channel?.topic ?? "", slowmodeSeconds: Number(data.channel?.slowmodeSeconds ?? 0), isNsfw: Boolean(data.channel?.isNsfw) });
  ...
  async function saveChannelSettings() {
    ...
    const updated = {
      ...
      parentId: data.channel.parentId ?? null,
    ...
    setChannelSettings({ name: updated.name, parentId: updated.parentId, topic: updated.topic ?? "", slowmodeSeconds: updated.slowmodeSeconds, isNsfw: updated.isNsfw });
    onUpdated?.(updated);

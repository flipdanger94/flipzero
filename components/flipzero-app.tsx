    ...
    <><main className={`app-shell ${showMembers ? "" : "members-hidden"} ${mobileChannelsOpen ? "mobile-channels-open" : ""} ${platformView ? "platform-view-active" : ""}`}>
      ...
      <aside className={`channel-panel ${activeSpace ? "has-server-banner" : ""}`}>
        ...
        <div className="channel-scroll">
          {activeSpace ? <>{activeSpace.categories.map((category) => <ChannelGroup key={category.id} title={category.name.toLocaleUpperCase("ru")} onAdd={canManageChannels ? () => setCreateChannelTarget({ kind: "text", parentId: category.id }) : undefined} onEdit={canManageChannels ? () => renameCategory(category) : undefined} onDelete={canManageChannels ? () => deleteCategory(category) : undefined}>{activeSpace.channels.filter((channel) => channel.parentId === category.id).map((channel) => <Channel key={channel.id} active={activeChannel === channel.name} icon={["voice", "stage"].includes(channel.kind) ? <Volume2 size={17} /> : channel.name === "добро-пожаловать" ? <BookOpen size={17} /> : <Hash size={17} />} label={channel.name} voice={["voice", "stage"].includes(channel.kind)} participants={voicePresence[channel.id]} onSelect={() => selectChannel(channel.name, channel.id, activeSpace.id)} onManage={canManageChannels ? () => setPermissionsChannel(channel) : undefined} onDelete={canManageChannels ? () => deleteChannel(channel) : undefined} />)}</ChannelGroup>)}{activeSpace.channels.some((channel) => !channel.parentId) ? <ChannelGroup title="БЕЗ КАТЕГОРИИ" onAdd={canManageChannels ? () => setCreateChannelTarget({ kind: "text", parentId: null }) : undefined}>{activeSpace.channels.filter((channel) => !channel.parentId).map((channel) => <Channel key={channel.id} active={activeChannel === channel.name} icon={["voice", "stage"].includes(channel.kind) ? <Volume2 size={17} /> : <Hash size={17} />} label={channel.name} voice={["voice", "stage"].includes(channel.kind)} participants={voicePresence[channel.id]} onSelect={() => selectChannel(channel.name, channel.id, activeSpace.id)} onManage={canManageChannels ? () => setPermissionsChannel(channel) : undefined} onDelete={canManageChannels ? () => deleteChannel(channel) : undefined} />)}</ChannelGroup> : null}{canManageChannels ? <button className="category-add" onClick={() => setShowCreateCategory(true)}><Plus size={14} /> Новая категория</button> : null}</> : <div className="space-empty"><strong>Здесь пока пусто</strong><span>Создайте первое пространство, чтобы открыть каналы и роли.</span><button onClick={() => setShowCreateSpace(true)}>Создать пространство</button></div>}
        </div>

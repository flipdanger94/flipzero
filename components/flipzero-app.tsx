...
type SpaceMember = { userId: string; nickname: string | null; username: string | null; displayName: string; avatarUrl: string | null; level: number; roleIds?: string[]; };
...
export default function Home({ initialSpaceId, initialChannelId }: { initialSpaceId?: string; initialChannelId?: string } = {}) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/v1/spaces/${encodeURIComponent(activeSpaceId)}/members?limit=80`, { cache: "no-store" }).then((response) => response.ok ? response.json() : { members: [], total: 0, nextCursor: null }),
      fetch(`/api/v1/spaces/${encodeURIComponent(activeSpaceId)}/roles`, { cache: "no-store" }).then((response) => response.ok ? response.json() : { roles: [] }),
    ])
      .then(([data, roleData]) => {
        if (cancelled) return;
        setSpaceMembers((data.members ?? []) as SpaceMember[]);
        setSpaceRoles(roleData.roles ?? []);
  ...
  return (
    <><main className={`app-shell ${showMembers ? "" : "members-hidden"} ${mobileChannelsOpen ? "mobile-channels-open" : ""} ${platformView ? "platform-view-active" : ""}`}>
      <nav className="space-rail" aria-label="Сообщества">
        ...
        <div className="rail-bottom"><a className="rail-action" href="/developers/console" aria-label="Платформа разработчиков" title="Платформа разработчиков"><Code2 size={20} /></a><button className="rail-action" aria-label="Состояние системы" onClick={() => setShowSystemStatus(true)}><HelpCircle size={20} /></button></div>
      </nav>

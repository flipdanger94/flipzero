...
export default function Home({ initialSpaceId, initialChannelId }: { initialSpaceId?: string; initialChannelId?: string } = {}) {
  ...
  const connectedVoiceChannel = voiceSession?.connected ? activeSpace?.channels.find((channel) => channel.id === voiceSession.channelId) ?? null : null;
  const voiceHostChannel = connectedVoiceChannel ?? activeVoiceChannel;
  const memberGroups = (() => {
  ...
  return (
    <><main className={`app-shell ${showMembers && activeSpace ? "" : "members-hidden"} ${mobileChannelsOpen ? "mobile-channels-open" : ""} ${platformView ? "platform-view-active" : ""}`}>
      <nav className="space-rail" aria-label="Сообщества">
        ...
        <div className="rail-bottom"><button className={`rail-action ${platformView==="clans"?"active":""}`} aria-label="Кланы" title="Кланы" onClick={()=>{setPlatformView("clans");setMobileChannelsOpen(false)}}><Swords size={20}/></button><button className="rail-action" aria-label="Состояние системы" title="Состояние системы" onClick={() => openExclusiveOverlay(() => setShowSystemStatus(true))}><HelpCircle size={20} /></button></div>
      </nav>

      {user && platformView ? <section className="platform-workspace" aria-label={platformView === "social" ? "Личное пространство" : platformView === "clans" ? "Кланы" : "Панель администратора"}>{platformView === "social" ? <SocialHubDialog key={`${socialRoute.tab}:${socialRoute.userId ?? ""}:${socialRoute.nonce}`} currentUserId={user.id} embedded initialTab={socialRoute.tab} initialUserId={socialRoute.userId} isAdmin={user.platformRole === "admin"} onOpenAdmin={() => setPlatformView("admin")} /> : platformView === "clans" ? <ClanHub currentUserId={user.id} onOpenDirect={(userId)=>{setSocialRoute(route=>({tab:"messages",userId,nonce:route.nonce+1}));setPlatformView("social")}}/> : user.platformRole === "admin" ? <AdminDialog embedded /> : null}</section> : null}
      ...
      <section className="chat-panel">
        <header className="chat-header"><button className="mobile-menu-button" aria-label="Открыть сообщества и каналы" onClick={() => setMobileChannelsOpen(true)}><Menu size={20} /></button><div className="channel-title"><Hash size={21} /><strong>{activeRouteChannel?.name ?? "Чат"}</strong><span>{activeRouteChannel?.topic ?? (activeSpace ? activeSpace.name : "Выберите пространство")}</span></div><div className="header-actions"><button aria-label="Единый поиск" title="Поиск Ctrl+K" onClick={()=>setGlobalSearchOpen(true)}><Search size={18}/></button><button className={`channel-share-action ${channelLinkCopied ? "is-active link-copied" : ""}`} aria-label={channelLinkCopied ? "Ссылка на канал скопирована" : "Поделиться ссылкой на канал"} title={channelLinkCopied ? "Скопировано" : "Поделиться ссылкой на канал"} onClick={shareChannelLink} disabled={!activeRouteChannel}>{channelLinkCopied ? <Check size={18} /> : <><Copy className="desktop-copy-icon" size={18} /><Share2 className="mobile-share-icon" size={18} /></>}</button><NotificationCenter onOpenMessages={(userId) => { setSocialRoute((route) => ({ tab: "messages", userId, nonce: route.nonce + 1 })); setPlatformView("social"); }} onOpenFriends={() => { setSocialRoute((route) => ({ tab: "friends", userId: null, nonce: route.nonce + 1 })); setPlatformView("social"); }} onOpenClans={()=>setPlatformView("clans")} /><button className={showMembers ? "is-active" : ""} aria-label={showMembers ? "Скрыть участников" : "Показать участников"} aria-pressed={showMembers} onClick={() => setShowMembers((value) => !value)}><Users size={19} /></button><label className="search-box"><Search size={16} /><input ref={searchInputRef} aria-label="Поиск" placeholder="Поиск · Ctrl+K" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} /></label></div></header>
        {voiceHostChannel ? <div className={`voice-persistent-host ${activeVoiceChannel?.id === voiceHostChannel.id ? "is-visible" : "is-background"}`}>
          <VoiceRoom key={voiceHostChannel.id} channelId={voiceHostChannel.id} channelName={voiceHostChannel.name} spaceName={activeSpace?.name ?? ""} autoJoin={Boolean(activeVoiceChannel?.id === voiceHostChannel.id)} presence={voicePresence[voiceHostChannel.id] ?? []} initialStreamId={voiceStreamTarget?.channelId===voiceHostChannel.id?voiceStreamTarget.participantId:""} onPresenceChange={(participants) => setVoicePresence((current) => {

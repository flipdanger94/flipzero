...
import { Bell, BookOpen, Check, ChevronDown, Columns3, Copy, Hash, Headphones, HelpCircle, LoaderCircle, Lock, Menu, MessagesSquare, Mic, MicOff, MonitorUp, PhoneOff, Plus, Radio, Share2, ShieldCheck, Signal, Trash2, Video, Volume2 } from "lucide-react";
...
export default function Home({ initialSpaceId, initialChannelId }: { initialSpaceId?: string; initialChannelId?: string } = {}) {
  ...
  const [voiceSession, setVoiceSession] = useState<{ connected: boolean; channelId: string; channelName: string; spaceName?: string; sessionKind?: "channel"|"clan"; sessionTargetId?: string } | null>(null);
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      const detail = (event as CustomEvent<{ connected?: boolean; channelId?: string; channelName?: string; spaceName?: string; sessionKind?: "channel"|"clan"; sessionTargetId?: string }>).detail;
      if (!detail?.connected) { setVoiceSession(null); return; }
      if (detail.channelId && detail.channelName) setVoiceSession({ connected: true, channelId: detail.channelId, channelName: detail.channelName, spaceName: detail.spaceName, sessionKind: detail.sessionKind, sessionTargetId: detail.sessionTargetId });
    };
  ...
  const voiceSessionSpace = voiceSession?.connected && voiceSession.sessionKind !== "clan" ? userSpaces.find((space) => space.channels.some((channel) => channel.id === voiceSession.channelId)) ?? null : null;
  const connectedVoiceChannel = voiceSessionSpace?.channels.find((channel) => channel.id === voiceSession?.channelId && ["voice", "stage"].includes(channel.kind)) ?? null;
  const voiceHostChannel = activeVoiceChannel ?? connectedVoiceChannel;
  const voiceHostSpace = activeVoiceChannel ? activeSpace : voiceSessionSpace;
  const memberGroups = (() => {
  ...
  return (
    <><main className={`app-shell ${showMembers && activeSpace ? "" : "members-hidden"} ${mobileChannelsOpen ? "mobile-channels-open" : ""} ${platformView ? "platform-view-active" : ""} ${voiceSession?.connected ? "has-voice-session" : ""}`}>
      <nav className="space-rail" aria-label="Сообщества">
        ...
        <button className="rail-action add-space" aria-label="Добавить сообщество" title="Создать пространство" onClick={() => openExclusiveOverlay(() => setShowCreateSpace(true))}><AppIcon name="plus" size={22}/></button>
        <button className="rail-action discover" aria-label="Обзор сообществ" title="Обзор пространств" onClick={() => openExclusiveOverlay(() => setShowDiscovery(true))}><AppIcon name="compass" size={21}/></button>
        <div className="rail-bottom"><button className={`rail-action ${platformView==="clans"?"active":""}`} aria-label="Кланы" title="Кланы" onClick={()=>openPlatformView("clans")}><AppIcon name="clans" size={20}/></button><button className="rail-action" aria-label="Состояние системы" title="Состояние системы" onClick={() => openExclusiveOverlay(() => setShowSystemStatus(true))}><HelpCircle size={20} /></button></div>
      </nav>
      ...
      {voiceSession?.connected ? <div className="mobile-voice-session" role="status"><button type="button" onClick={() => {
        if(voiceSession.sessionKind==="clan"&&voiceSession.sessionTargetId){
          openPlatformView("clans");
          window.history.replaceState(null,"",`/app?clan=${encodeURIComponent(voiceSession.sessionTargetId)}&voice=1`);
          window.setTimeout(()=>window.dispatchEvent(new CustomEvent("flipzero:open-clan-voice",{detail:voiceSession.sessionTargetId})),80);
          return;
        }
        const channel=activeSpace?.channels.find((item)=>item.id===voiceSession.channelId); if(channel) openVoiceChannel(channel);
      }}><Signal size={15}/><span><strong>Голосовая связь подключена</strong><small>{voiceSession.channelName}{voiceSession.spaceName?` · ${voiceSession.spaceName}`:""}</small></span></button><button type="button" aria-label="Отключиться от голосового канала" onClick={()=>window.dispatchEvent(new CustomEvent("flipzero:voice-control",{detail:{type:"leave"}}))}><PhoneOff size={17}/></button></div> : null}
      <nav className="mobile-tab-bar mobile-tab-bar-six" aria-label="Основная навигация"><button className={platformView === "social" ? "active" : ""} onClick={() => openPlatformView("social")}><AppIcon name="messages" size={20}/><span>Личное</span></button><button className={platformView==="clans"?"active":""} aria-label="Кланы" title="Кланы" onClick={()=>openPlatformView("clans")}><AppIcon name="clans" size={20}/><span>Кланы</span></button><button onClick={() => openExclusiveOverlay(() => setShowDiscovery(true))}><AppIcon name="compass" size={20}/><span>Обзор</span></button><button className={mobileChannelsOpen ? "active" : ""} onClick={() => { setPlatformView(null); setMobileChannelsOpen(true); if(activeSpace&&activeRouteChannel) window.history.pushState({}, "", `/channels/${activeSpace.id}/${activeRouteChannel.id}`); }}><AppIcon name="home" size={20}/><span>Пространства</span></button><button className={!platformView && !mobileChannelsOpen ? "active" : ""} onClick={() => { setPlatformView(null); setMobileChannelsOpen(false); if(activeSpace&&activeRouteChannel&&window.location.pathname!==`/channels/${activeSpace.id}/${activeRouteChannel.id}`) window.history.pushState({}, "", `/channels/${activeSpace.id}/${activeRouteChannel.id}`); }}><AppIcon name="channel" size={20}/><span>Чат</span></button><button onClick={() => { setAccountSettingsSection("profile"); setMobileChannelsOpen(false); openExclusiveOverlay(() => setShowAccountSettings(true)); }}><AppIcon name="user" size={20}/><span>Профиль</span></button></nav>
...
function Channel({
  ...
  return <div className={`channel-row ${active ? "active" : ""} ${full ? "voice-channel-full" : ""}`}>
    ...
    {onManage ? <button className="channel-manage" aria-label={`Настроить права канала ${label}`} title={`Настроить права канала ${label}`} onClick={onManage}><AppIcon name="settings" size={14}/></button> : null}
    ...
    {context ? <div className="voice-user-context-menu" style={{ left: context.x, top: context.y }} role="menu" aria-label={`Действия для ${context.participant.name}`}>
      ...
      <button type="button" role="menuitem" onClick={()=>{onOpenProfile?.(context.participant);setContext(null)}}><AppIcon name="user" size={15}/>Профиль</button>

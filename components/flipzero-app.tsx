...
import { BookOpen, Check, ChevronDown, ChevronUp, CirclePlus, Code2, Compass, Copy, Gem, Gift, Hash, Headphones, HelpCircle, Home as HomeIcon, Image as ImageIcon, LoaderCircle, Menu, MessageCircle, Mic, MicOff, MonitorUp, Plus, Search, SendHorizontal, Settings, Settings2, Share2, ShieldCheck, Smile, Sparkles, Trash2, UserRound, Users, Video, Volume2, X } from "lucide-react";
...
type ApiSpace = { id: string; ownerId?: string; permissions?: number; name: string; slug: string; description: string | null; iconUrl?: string | null; bannerUrl?: string | null; visibility?: string; accentColor: string; categories: ApiCategory[]; channels: ApiChannel[] };
...
export default function Home({ initialSpaceId, initialChannelId }: { initialSpaceId?: string; initialChannelId?: string } = {}) {
  ...
  const isSpaceOwner = Boolean(activeSpace && activeSpace.ownerId === user?.id);
  const spacePermissions = activeSpace?.permissions ?? 0;
  const canManageChannels = Boolean(activeSpace && (isSpaceOwner || hasPermission(spacePermissions, Permission.ManageChannels)));
  const canManageRoles = Boolean(activeSpace && (isSpaceOwner || hasPermission(spacePermissions, Permission.ManageRoles)));
  ...
  function addChannel(channel: CreatedChannel) {
    ...
    if (!["voice", "stage"].includes(channel.kind)) selectChannel(channel.name, channel.id, channel.spaceId);
    setCreateChannelTarget(null);
  }

  function updateChannel(updated: ApiChannel, previousName?: string) {
    setUserSpaces((current) => current.map((space) => space.id === activeSpaceId ? {
      ...space,
      channels: space.channels.map((channel) => channel.id === updated.id ? { ...channel, ...updated } : channel),
    } : space));
    setPermissionsChannel((current) => current?.id === updated.id ? { ...current, ...updated } : current);
    if (previousName && activeChannel === previousName) setActiveChannel(updated.name);
  }
  ...
  async function moveCategory(categoryId: string, direction: -1 | 1) {
    if (!activeSpace) return;
    const ordered = [...activeSpace.categories].sort((a, b) => a.position - b.position);
    const index = ordered.findIndex((item) => item.id === categoryId);
    const target = index + direction;
    if (index < 0 || target < 0 || target >= ordered.length) return;
    [ordered[index], ordered[target]] = [ordered[target], ordered[index]];
    const response = await fetch(`/api/v1/spaces/${activeSpace.id}/categories`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ action: "reorder", orderedIds: ordered.map((item) => item.id) }),
    });
    const result = await response.json().catch(() => null);
    if (!response.ok) { setAppNotice({ message: result?.message ?? "Не удалось изменить порядок категорий.", tone: "error" }); return; }
    setUserSpaces((current) => current.map((space) => space.id === activeSpace.id ? {
      ...space,
  ...
  async function moveChannel(channel: ApiChannel, direction: -1 | 1) {
    if (!activeSpace) return;
    const siblings = activeSpace.channels.filter((item) => item.parentId === channel.parentId).sort((a, b) => (a.position ?? 0) - (b.position ?? 0));
    const index = siblings.findIndex((item) => item.id === channel.id);
    const target = index + direction;
    if (index < 0 || target < 0 || target >= siblings.length) return;
    [siblings[index], siblings[target]] = [siblings[target], siblings[index]];
    const response = await fetch(`/api/v1/spaces/${activeSpace.id}/channels`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ action: "reorder", parentId: channel.parentId, orderedIds: siblings.map((item) => item.id) }),
    });
    const result = await response.json().catch(() => null);
    if (!response.ok) { setAppNotice({ message: result?.message ?? "Не удалось изменить порядок каналов.", tone: "error" }); return; }
    const positions = new Map(siblings.map((item, position) => [item.id, position]));
    setUserSpaces((current) => current.map((space) => space.id === activeSpace.id ? {
      ...space,
      channels: space.channels.map((item) => positions.has(item.id) ? { ...item, position: positions.get(item.id)! } : item),
    } : space));
  }
  ...
  async function renameCategory(category: ApiCategory) {
    ...
    const result = await response.json().catch(() => null);
    if (!response.ok) { setAppNotice({ message: result?.message ?? "Не удалось переименовать категорию.", tone: "error" }); return; }
    setUserSpaces((current) => current.map((space) => space.id === activeSpace.id ? {
      ...space,
      categories: space.categories.map((item) => item.id === category.id ? { ...item, ...result.category } : item),
    } : space));
  }
  ...
  async function openJoinedSpace(spaceId: string, preferredChannelId?: string | null) {
    ...
    if (selected) {
      const preferred = preferredChannelId ? selected.channels.find((channel) => channel.id === preferredChannelId) : null;
      const first = preferred ?? selected.channels.find((channel) => channel.kind === "text") ?? selected.channels.find((channel) => !["voice", "stage"].includes(channel.kind));
    ...
    <><main className={`app-shell ${showMembers ? "" : "members-hidden"} ${mobileChannelsOpen ? "mobile-channels-open" : ""} ${platformView ? "platform-view-active" : ""}`}>
      ...
      <aside className={`channel-panel ${activeSpace ? "has-server-banner" : ""}`}>
        ...
        <div className="channel-scroll">
          {activeSpace ? <>{activeSpace.categories.map((category, categoryIndex) => <ChannelGroup key={category.id} title={category.name.toLocaleUpperCase("ru")} onAdd={canManageChannels ? () => setCreateChannelTarget({ kind: "text", parentId: category.id }) : undefined} onMoveUp={canManageChannels && categoryIndex > 0 ? () => void moveCategory(category.id, -1) : undefined} onMoveDown={canManageChannels && categoryIndex < activeSpace.categories.length - 1 ? () => void moveCategory(category.id, 1) : undefined} onEdit={canManageChannels ? () => renameCategory(category) : undefined} onDelete={canManageChannels ? () => deleteCategory(category) : undefined}>{activeSpace.channels.filter((channel) => channel.parentId === category.id).sort((a, b) => (a.position ?? 0) - (b.position ?? 0)).map((channel, channelIndex, siblings) => <Channel key={channel.id} active={activeChannel === channel.name} icon={["voice", "stage"].includes(channel.kind) ? <Volume2 size={17} /> : channel.name === "добро-пожаловать" ? <BookOpen size={17} /> : <Hash size={17} />} label={channel.name} voice={["voice", "stage"].includes(channel.kind)} participants={voicePresence[channel.id]} onSelect={() => selectChannel(channel.name, channel.id, activeSpace.id)} onMoveUp={canManageChannels && channelIndex > 0 ? () => void moveChannel(channel, -1) : undefined} onMoveDown={canManageChannels && channelIndex < siblings.length - 1 ? () => void moveChannel(channel, 1) : undefined} onManage={canManageChannels ? () => setPermissionsChannel(channel) : undefined} onDelete={canManageChannels ? () => deleteChannel(channel) : undefined} />)}</ChannelGroup>)}{activeSpace.channels.some((channel) => !channel.parentId) ? <ChannelGroup title="БЕЗ КАТЕГОРИИ" onAdd={canManageChannels ? () => setCreateChannelTarget({ kind: "text", parentId: null }) : undefined}>{activeSpace.channels.filter((channel) => !channel.parentId).sort((a, b) => (a.position ?? 0) - (b.position ?? 0)).map((channel, channelIndex, siblings) => <Channel key={channel.id} active={activeChannel === channel.name} icon={["voice", "stage"].includes(channel.kind) ? <Volume2 size={17} /> : <Hash size={17} />} label={channel.name} voice={["voice", "stage"].includes(channel.kind)} participants={voicePresence[channel.id]} onSelect={() => selectChannel(channel.name, channel.id, activeSpace.id)} onMoveUp={canManageChannels && channelIndex > 0 ? () => void moveChannel(channel, -1) : undefined} onMoveDown={canManageChannels && channelIndex < siblings.length - 1 ? () => void moveChannel(channel, 1) : undefined} onManage={canManageChannels ? () => setPermissionsChannel(channel) : undefined} onDelete={canManageChannels ? () => deleteChannel(channel) : undefined} />)}</ChannelGroup> : null}{canManageChannels ? <button className="category-add" onClick={() => setShowCreateCategory(true)}><Plus size={14} /> Новая категория</button> : null}</> : <div className="space-empty"><strong>Здесь пока пусто</strong><span>Создайте первое пространство, чтобы открыть каналы и роли.</span><button onClick={() => setShowCreateSpace(true)}>Создать пространство</button></div>}
        </div>

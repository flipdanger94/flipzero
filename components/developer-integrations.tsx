...
export function DeveloperIntegrations({ appId }: { appId: string }) {
  ...
  return <section className="developer-integrations">
    ...
    {tab === "bot" ? <div className="developer-section">
      <div className="developer-section-head"><div><small>BOT INSTALLATIONS</small><h4>Установка приложения</h4><p>Подключите приложение только к тем пространствам, события которых оно должно получать. Пока приложение не установлено, webhooks пространства ему не отправляются.</p></div></div><div className="bot-install-link"><div><span>Install link</span><code>{installPath}</code></div><button onClick={copyInstallUrl} title="Копировать ссылку установки">{installCopied ? <Check size={14} /> : <Copy size={14} />}</button></div>
      <div className="bot-install-list">{spaces.map((space) => <article key={space.id} className={space.installation ? "installed" : ""}><span className="bot-space-icon" style={{ background: space.accentColor }}>{space.name.slice(0, 2).toLocaleUpperCase("ru")}</span><div><strong>{space.name}</strong><small>{space.installation ? "Приложение установлено · events:read" : "Нет доступа к событиям пространства"}</small></div><button disabled={working} onClick={() => toggleInstallation(space)}>{space.installation ? "Удалить" : "Установить"}</button></article>)}{!spaces.length ? <div className="developer-empty">Для установки нужно пространство, владельцем которого вы являетесь.</div> : null}</div>
    </div> : null}

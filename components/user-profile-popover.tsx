    ...
    <section ref={miniRef} className={`fz-mini-profile effect-${p?.cosmetics?.profile_effect??"none"}`} style={{...position,"--profile-accent":p?.accentColor??"#7c5cff"} as CSSProperties} role="dialog" aria-label={`Профиль ${displayName}`}>
      {!p?<div className="fz-mini-profile-state">{error?<><ShieldCheck/><strong>Профиль недоступен</strong><small>{error}</small></>:<><LoaderCircle className="spin"/><span>Загрузка профиля…</span></>}</div>:<>
        ...
        <div className="fz-mini-body">
          ...
          <div className="fz-mini-xp"><span><b>{p.globalXp} XP</b><small>{p.globalLevel>=100?"Максимальный уровень":`До уровня ${p.globalLevel+1}: ${p.xpToNextLevel} XP`}</small></span><i><b style={{width:`${p.globalLevel>=100?100:Math.max(0,Math.min(100,((p.globalXp-p.currentLevelXp)/Math.max(1,p.nextLevelXp-p.currentLevelXp))*100))}%`}}/></i></div><div className="fz-mini-badges"><span>LVL {p.globalLevel}</span>{p.cosmetics?.badge?<span className={`store-profile-badge badge-${p.cosmetics.badge}`}>✦ {p.cosmetics.badge}</span>:null}{p.badges?.map(badge=><span key={badge.id} title={badge.name} aria-label={badge.name}>{badge.icon} {badge.name}</span>)}{p.isFriend?<span>ДРУГ</span>:null}{p.presence==="online"?<span>ONLINE</span>:null}</div>

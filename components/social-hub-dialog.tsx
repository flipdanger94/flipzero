...
export function SocialHubDialog({ currentUserId, initialTab = "messages", initialUserId, onClose, embedded = false, isAdmin = false, onOpenAdmin }: { currentUserId: string; initialTab?: "messages" | "friends" | "superflip"; initialUserId?: string | null; onClose?: () => void; embedded?: boolean; isAdmin?: boolean; onOpenAdmin?: () => void }) {
  ...
  const content=<section ref={modalRef} tabIndex={embedded?undefined:-1} className={`social-hub social-hub-v2 ${embedded?"social-hub-embedded":""}`} role={embedded?"region":"dialog"} aria-modal={embedded?undefined:true}>
    <aside className="social-nav-rail">
      ...
      <div className="social-dialog-list">{filteredConversations.length?filteredConversations.map((item)=><button key={item.id} className={tab==="messages"&&active?.id===item.id?"active":""} onClick={()=>{setTab("messages");setMessages([]);setNotice("");followLatestRef.current=true;setProfileVisible(true);setActive(item)}}><Avatar person={item.other}/><span><strong>{item.other.displayName}</strong><small>{item.lastMessage?.text??"Новый диалог"}</small></span>{item.unread?<b>{item.unread}</b>:null}</button>):<p>Диалогов пока нет.</p>}</div>
    </aside>
    ...
    {profilePopup?<UserProfilePopover userId={profilePopup.person.id} displayName={profilePopup.person.displayName} anchor={profilePopup.anchor} onClose={()=>setProfilePopup(null)} onOpenDirect={(userId)=>{const person=[...friends,...conversations.map((item)=>item.other),...results].find((item)=>item.id===userId)??profilePopup.person;setProfilePopup(null);void openChat(person)}}/>:null}
  </section>;

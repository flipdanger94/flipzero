...
type Person = { clan?:ClanTagData|null; id: string; username: string; displayName: string; avatarUrl?: string | null; presence?: string };
...
export function SocialHubDialog({ currentUserId, initialTab = "messages", initialUserId, onClose, embedded = false, isAdmin = false, onOpenAdmin }: { currentUserId: string; initialTab?: "messages" | "friends" | "superflip"; initialUserId?: string | null; onClose?: () => void; embedded?: boolean; isAdmin?: boolean; onOpenAdmin?: () => void }) {
  ...
  const content=<section ref={modalRef} tabIndex={embedded?undefined:-1} className={`social-hub social-hub-v2 ${embedded?"social-hub-embedded":""}`} role={embedded?"region":"dialog"} aria-modal={embedded?undefined:true}>
    <aside className="social-nav-rail">
      ...
      <div className="social-dialog-list">{filteredConversations.length?filteredConversations.map((item)=><button key={item.id} className={tab==="messages"&&active?.id===item.id?"active":""} onClick={()=>{setTab("messages");setMessages([]);setNotice("");followLatestRef.current=true;setProfileVisible(true);setActive(item)}}><Avatar person={item.other}/><span><strong>{item.other.displayName}</strong><ClanTag clan={item.other.clan}/><small>{item.lastMessage?.text??"Новый диалог"}</small></span>{item.unread?<b>{item.unread}</b>:null}</button>):<p>Диалогов пока нет.</p>}</div>
    </aside>
    <div className="social-main-v2">
      ...
      :tab==="messages"?<div className={`direct-layout social-direct-main ${active?"has-profile":""} ${profileVisible?"profile-visible":"profile-hidden"}`}>
        <section className={dragging?"is-dragging":""} onDragEnter={(event)=>{event.preventDefault();setDragging(true)}} onDragOver={(event)=>event.preventDefault()} onDragLeave={(event)=>{if(event.currentTarget===event.target)setDragging(false)}} onDrop={onDrop}>
          ...
          <form className="direct-composer" onSubmit={sendMessage}>{pendingFiles.length?<div className="direct-attachment-preview">{pendingFiles.map((item)=><article key={item.id}>{item.previewUrl?<div className="dm-preview-image" style={{backgroundImage:`url("${item.previewUrl}")`}}/>:item.type==="audio"?<Mic size={22}/>:<FileIcon size={22}/>}<span><strong>{item.file.name}</strong><small>{Math.ceil(item.file.size/1024)} КБ{item.duration?` · ${item.duration} сек.`:""}</small></span><button type="button" onClick={()=>removePending(item.id)} aria-label="Удалить вложение"><X size={14}/></button></article>)}</div>:null}<div className="direct-composer-row"><input ref={fileInputRef} type="file" hidden multiple accept="image/png,image/jpeg,image/webp,image/gif,audio/*,.pdf,.txt,.zip" onChange={(event)=>{addFiles([...(event.currentTarget.files??[])]);event.currentTarget.value=""}}/><button type="button" onClick={()=>fileInputRef.current?.click()} title="Прикрепить файл" aria-label="Прикрепить файл"><FilePlus2 size={18}/></button><textarea name="text" rows={1} value={draft} onChange={(event)=>setDraft(event.target.value)} placeholder={superflip?.active?"Написать сообщение · SuperFlip до 4000 символов":"Написать сообщение · до 1000 символов"} aria-invalid={overLimit}/><button type="button" className={recording?"is-recording":""} onClick={recording?stopRecording:()=>void startRecording()} title={recording?"Остановить запись":"Голосовое сообщение"} aria-label={recording?"Остановить запись":"Голосовое сообщение"}><Mic size={18}/>{recording?<small>{recordSeconds}</small>:null}</button><button type="button" onClick={()=>setEmojiOpen((value)=>!value)} title="Смайлики" aria-label="Смайлики"><Smile size={18}/></button><button type="button" disabled title="Стикеры — скоро" aria-label="Стикеры — скоро"><Sparkles size={18}/></button><button className="direct-send" disabled={sending||overLimit||(!draft.trim()&&!pendingFiles.length)} aria-label="Отправить сообщение"><SendHorizontal size={18}/></button></div><div className="direct-composer-meta"><span>{superflip?.active?"SuperFlip: расширенный лимит 4000 символов":"Free: 1000 символов · SuperFlip даёт до 4000"}</span><b className={overLimit?"over-limit":""}>{draft.length}/{messageLimit}</b></div>{emojiOpen?<div className="direct-emoji-picker">{quickEmoji.map((emoji)=><button type="button" key={emoji} onClick={()=>insertEmoji(emoji)}>{emoji}</button>)}</div>:null}</form>{notice?<p className="direct-notice" role="alert">{notice}</p>:null}
          </>:<div className="social-empty"><MessageCircle size={32}/><strong>Выберите диалог</strong><span>Выберите собеседника слева или создайте новое сообщение.</span><button onClick={()=>{setTab("friends");setFriendView("add")}}>Новое сообщение</button></div>}
        </section>
...
function FriendRow({person,subtitle,actions,onProfile}:{person:Person;subtitle?:string;actions:ReactNode;onProfile:(anchor:{x:number;y:number})=>void}){return <article className="friend-row-v2"><button type="button" className="friend-profile-trigger" onClick={(event)=>{const rect=event.currentTarget.getBoundingClientRect();onProfile({x:rect.left,y:rect.bottom+6})}} aria-label={`Открыть профиль ${person.displayName}`}><Avatar person={person}/><span><strong>{person.displayName}</strong><ClanTag clan={person.clan}/><small>@{person.username} · {subtitle??(person.presence==="online"?"В сети":"Не в сети")}</small></span></button><div className="friend-row-actions">{actions}</div></article>}

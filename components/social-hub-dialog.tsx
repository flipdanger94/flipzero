...
export function SocialHubDialog({ currentUserId, initialTab = "messages", initialUserId, onClose, embedded = false, isAdmin = false, onOpenAdmin }: { currentUserId: string; initialTab?: "messages" | "friends" | "superflip"; initialUserId?: string | null; onClose?: () => void; embedded?: boolean; isAdmin?: boolean; onOpenAdmin?: () => void }) {
  ...
  useEffect(() => { if (!initialUserId || loading) return; const timer=window.setTimeout(()=>{const existing=conversations.find(item=>item.other.id===initialUserId); if(existing){setActive(existing);setTab("messages");return} void fetch(`/api/v1/users/${initialUserId}/profile`).then(r=>r.json()).then(d=>{if(d.profile){setActive({id:"",other:{id:d.profile.id,username:d.profile.username,displayName:d.profile.displayName,avatarUrl:d.profile.avatarUrl,presence:d.profile.presence},unread:0,lastMessage:null});setTab("messages")}});},0); return()=>window.clearTimeout(timer); }, [initialUserId, loading, conversations]);

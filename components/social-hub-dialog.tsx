...
import {
  ...
  ShoppingBag, Sparkles, Star, UserCheck, UserPlus, UserRoundPlus, Users, Video, X, Zap,
} from "lucide-react";
...
export function SocialHubDialog({ currentUserId, initialTab = "messages", initialUserId, onClose, embedded = false, isAdmin = false, onOpenAdmin }: { currentUserId: string; initialTab?: "messages" | "friends" | "superflip" | "economy"; initialUserId?: string | null; onClose?: () => void; embedded?: boolean; isAdmin?: boolean; onOpenAdmin?: () => void }) {
  ...
  const content=<section ref={modalRef} tabIndex={embedded?undefined:-1} className={`social-hub social-hub-v2 ${embedded?"social-hub-embedded":""}`} role={embedded?"region":"dialog"} aria-modal={embedded?undefined:true}>
    <aside className="social-nav-rail">
      ...
      <button className={`social-economy-tab ${tab==="economy"?"active":""}`} onClick={()=>setTab("economy")}><ShoppingBag size={16}/>Магазин</button>
      <button className={`social-superflip-tab ${tab==="superflip"?"active premium":"premium"}`} onClick={()=>setTab("superflip")}><Crown size={16}/>SuperFlip</button>
    ...
    <div className={`social-main-v2 social-main-${tab}`}>
      <header><div><small>{tab==="messages"?"ЛИЧНЫЕ СООБЩЕНИЯ":tab==="friends"?"КОНТАКТЫ":tab==="economy"?"МАГАЗИН И ИНВЕНТАРЬ":"SUPERFLIP"}</small><h2>{tab==="messages"?(active?.other.displayName??"Сообщения"):tab==="friends"?"Друзья":tab==="economy"?"Магазин":"SuperFlip"}</h2></div>{onClose?<button onClick={onClose} aria-label="Закрыть"><X size={19}/></button>:null}</header>

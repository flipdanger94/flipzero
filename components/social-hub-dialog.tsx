...
import {
  ArrowLeft, Crown, File as FileIcon, FilePlus2, Image as ImageIcon, LoaderCircle, MapPin, MessageCircle,
    ...
    :tab==="messages"?<div className={`direct-layout ${active?"has-profile":""} ${profileVisible?"profile-visible":"profile-hidden"}`}>
      ...
      <section className={dragging?"is-dragging":""} onDragEnter={(event)=>{event.preventDefault();setDragging(true)}} onDragOver={(event)=>event.preventDefault()} onDragLeave={(event)=>{if(event.currentTarget===event.target)setDragging(false)}} onDrop={onDrop}>
        ...
        <form className="direct-composer" onSubmit={sendMessage}>
          {pendingFiles.length?<div className="direct-attachment-preview">{pendingFiles.map((item)=><article key={item.id}>{item.previewUrl?<div className="dm-preview-image" style={{backgroundImage:`url("${item.previewUrl}")`}}/>:item.type==="audio"?<Mic size={22}/>:<FileIcon size={22}/>}<span><strong>{item.file.name}</strong><small>{Math.ceil(item.file.size/1024)} КБ{item.duration?` · ${item.duration} сек.`:""}</small></span><button type="button" onClick={()=>removePending(item.id)} aria-label="Удалить вложение"><X size={14}/></button></article>)}</div>:null}

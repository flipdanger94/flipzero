...
export function ImageUpload({ kind, spaceId, label, currentUrl, onUploaded }: { kind: ImageKind; spaceId?: string; label: string; currentUrl?: string | null; onUploaded: (result: { user?: Record<string, unknown>; space?: Record<string, unknown> }) => void }) {
  ...
  return <><div className="image-upload"><label className="image-upload-button">{busy ? <LoaderCircle size={16} className="spin" /> : <Camera size={16} />} {busy ? "Обрабатываем…" : label}<input type="file" accept="image/png,image/jpeg,image/webp,image/gif,image/heic,image/heif,.heic,.heif" disabled={busy} onChange={(event) => { const selected = event.currentTarget.files?.[0]; if (selected?.type === "image/gif") void upload(selected); else if (selected) setCropFile(selected); event.currentTarget.value = ""; }} /></label>{currentUrl ? <button type="button" className="image-remove-button" aria-label={`Удалить: ${label}`} title={`Удалить: ${label}`} onClick={() => void remove()} disabled={busy}><Trash2 size={16} /></button> : null}{error ? <small role="alert" className="image-upload-error">{error}</small> : null}</div>{cropFile ? <ImageCropModal file={cropFile} aspect={kind === "spaceBanner" ? 16 / 9 : kind.endsWith("Banner") ? 4 : 1} onCancel={() => setCropFile(null)} onConfirm={async (cropped) => { setCropFile(null); await upload(cropped); }} /> : null}</>;
}

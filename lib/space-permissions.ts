...
import { channelOverrides, memberRoles, members, roles, spaces } from "@/db/schema";
...
export async function getChannelPermissions(channelId: string, userId: string) {
  ...
  const [base] = await db.select({ spaceId: members.spaceId, ownerId: spaces.ownerId })
    .from(members).innerJoin(spaces, eq(spaces.id, members.spaceId))
  ...
  const applicable = overrides.filter((item) =>
    (item.targetType === "member" && item.targetId === userId) ||
    (item.targetType === "role" && roleIds.includes(item.targetId))
  );
  let allow = 0, deny = 0;
  for (const item of applicable) { allow |= Number(item.allow); deny |= Number(item.deny); }
